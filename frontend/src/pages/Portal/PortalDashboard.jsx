    ...
    <>
      <div className="portal-page-header">
        ...
        <h1 className="portal-page-title">
          Bienvenido{integrante?.nick ? `, ${integrante.nick}` : integrante?.nombre ? `, ${integrante.nombre.split(' ')[0]}` : ''}
        </h1>
      </div>
      ...
      <div className="portal-stats-grid">
        <div className="portal-stat-card">
          <span className="portal-stat-num">{resumen?.pagadas ?? '—'}</span>
          <span className="portal-stat-label">Cuotas pagadas {anioActual}</span>
        </div>
        <div className="portal-stat-card">
          <span className="portal-stat-num" style={{ color: resumen?.pendientes > 0 ? '#b8952a' : '#52a852' }}>
            {resumen?.pendientes ?? '—'}
          </span>
          <span className="portal-stat-label">Cuotas pendientes</span>
        </div>
        <div className="portal-stat-card">
          <span className="portal-stat-num" style={{ color: deudas.length > 0 ? '#cc2222' : '#52a852' }}>
            {deudas.length}
          </span>
      ...
      <div className="portal-grid-2">
        ...
        <div className="portal-card">
          ...
          {mesesPendientes.length === 0 ? (
            <p style={{ color: '#52a852', fontSize: '0.9rem', margin: 0 }}>
              ✓ Al día con todas las cuotas del año
            </p>
          ) : (
            <>
              {mesesPendientes.slice(0, 5).map(m => (
                <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid #1a1a1a' }}>
                  <span style={{ fontSize: '0.9rem', color: '#aaa' }}>{m.mes_display}</span>
                  <span style={{ fontSize: '0.85rem', color: '#b8952a', fontWeight: 600 }}>
              ...
              <Link to="/portal/mis-cuotas" style={{ display: 'inline-block', marginTop: 12, fontSize: '0.8rem', color: '#cc2222', textDecoration: 'none', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
                Ver todas →
              </Link>
            </>
          )}
        ...
        <div className="portal-card">
          ...
          <InfoRow label="Rol"     value={integrante?.rol_display || integrante?.rol} />
          <InfoRow label="Teléfono" value={integrante?.telefono || '—'} />
          <Link to="/portal/mi-perfil" style={{ display: 'inline-block', marginTop: 12, fontSize: '0.8rem', color: '#cc2222', textDecoration: 'none', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
            Editar perfil →
          </Link>
        </div>
      </div>
      ...
      {deudas.length > 0 && (
        <div className="portal-card" style={{ borderColor: 'rgba(204,34,34,0.2)' }}>
          ...
          {deudas.map(d => (
            <div key={d.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '8px 0', borderBottom: '1px solid #1a1a1a' }}>
              <div>
                <div style={{ fontSize: '0.92rem', color: '#d0d0d0' }}>{d.descripcion}</div>
                {d.fecha_vencimiento && (
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: '0.95rem', fontWeight: 700, color: '#cc2222' }}>
                  ${Number(d.monto_pendiente).toLocaleString('es-CL')}
                </div>
                <div style={{ fontSize: '0.75rem', color: '#555' }}>pendiente</div>
              </div>
            </div>
          ))}
        </div>
...
function InfoRow({ label, value }) {
  return (
    <div style={{ display: 'flex', gap: 12, padding: '5px 0', borderBottom: '1px solid #111' }}>
      <span style={{ minWidth: 80, fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#555' }}>{label}</span>
